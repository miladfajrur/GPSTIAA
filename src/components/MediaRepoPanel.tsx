...
import { MediaRepo } from "../types";
import MonthYearInputMask from "./MonthYearInputMask";
import { parseMonthYearInput, toIndonesianMonthYearInput } from "../lib/utils";

export default function MediaRepoPanel() {
  const [items, setItems] = useState<MediaRepo[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<MediaRepo | undefined>(undefined);
  ...
  const openModal = (item?: MediaRepo) => {
  ...
  useEffect(() => {
    const q = query(
      collection(db, "media_repo"),
      where("tenantId", "==", "gpstiaa"),
      orderBy("bulan", "desc")
    );

    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data: MediaRepo[] = [];
      snapshot.forEach((doc) => {
        data.push({ id: doc.id, ...doc.data() } as MediaRepo);
      });
      setItems(data);
      setIsLoading(false);
    });
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedItem?.id) {
      await setDoc(doc(db, "media_repo", selectedItem.id), { ...data, updatedAt: serverTimestamp() }, { merge: true });
    } else {
      await setDoc(doc(collection(db, "media_repo")), { ...data, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    }
  ...
  const handleDelete = async () => {
    if (itemToDelete) {
      await deleteDoc(doc(db, "media_repo", itemToDelete));
  ...
  const uniqueCategories = Array.from(new Set([
    "Ibadah Hari Minggu", "Ibadah Kategorial", "Perayaan", "Event Khusus", "Lainnya",
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      <div className="p-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-1">Repositori Media (Foto & Video)</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">Penyimpanan link media berdasarkan bulan</p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      <div className="bg-slate-50 border-b border-slate-200 dark:bg-slate-800/80 dark:border-slate-700 px-4 py-3 flex gap-3 items-center">
        <select
      ...
      <div className="flex-1 overflow-auto p-4">
        {isLoading ? (
          ...
          <p className="text-center p-4 text-slate-500">Belum ada data repositori.</p>
        ) : (
              ...
              <div key={item.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow relative group">
                <div className="flex justify-between items-start mb-2">
                   <div>
                     ...
                     <span className="inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl max-w-md w-full p-6">
            <h3 className="text-lg font-bold mb-4 text-slate-800 dark:text-slate-100">{selectedItem ? "Edit" : "Tambah"} Media</h3>
            <form onSubmit={handleSave} className="space-y-4 text-sm text-slate-800 dark:text-slate-200">
              <div>
                ...
                <input required name="title" defaultValue={selectedItem?.title} placeholder="mis. Ibadah Paskah" className="w-full border dark:border-slate-600 rounded-lg p-2 dark:bg-slate-900" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    required 
                    name="category" 
                    defaultValue={selectedItem?.category || "Lainnya"}
                    list="category-options"
                    className="w-full border dark:border-slate-600 rounded-lg p-2 dark:bg-slate-900" 
                    placeholder="Pilih atau ketik kategori baru"
                  />
                  <datalist id="category-options">
                    {uniqueCategories.map((cat, idx) => (
                      <option key={`opt-${idx}`} value={cat} />
                    ))}
                  </datalist>
                </div>
                <div>
      ...
      {itemToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-sm w-full">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-6">Data ini akan dihapus secara permanen.</p>
